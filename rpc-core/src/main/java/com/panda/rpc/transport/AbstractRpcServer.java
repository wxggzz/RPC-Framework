...
import com.panda.rpc.entity.util.ReflectUtil;
...
public abstract class AbstractRpcServer implements RpcServer {
    ...
    public void scanServices(){
        ...
        logger.info("main()入口所在类名为："+mainClassName);
        Class<?> startClass;
        try {
            //获取启动类对应的实例对象
        ...
        System.out.println("before-basePackage："+basePackage);
        if("".equals(basePackage)){
            //获取启动类所在的包，因为服务类也放在这个包下面的
            basePackage = mainClassName.substring(0, mainClassName.lastIndexOf("."));
            System.out.println("after-basePackage："+basePackage);
        }
        //获取包下面的所有类Class对象，进行注册
